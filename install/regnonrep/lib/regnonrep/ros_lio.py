...
class IESKF:
    ...
    def propagate(self, omega: np.ndarray, accel: np.ndarray, dt: float):
        ...
        # Nominal state forward integration
        ...
        # Derived from the right-perturbation error-state kinematics on SO(3).
        F = np.eye(15)
        F[0:3, 3:6]   = np.eye(3) * dt                   # δp  ← δv
        F[3:6, 6:9]   = -(self.R @ _skew(accel_c)) * dt  # δv  ← δθ (accel rotation)
        F[3:6, 12:15] = -self.R * dt                      # δv  ← δba
        F[6:9, 6:9]   = so3_exp(-omega_c * dt)            # δθ  ← δθ (gyro kinematics)
        F[6:9, 9:12]  = -np.eye(3) * dt                   # δθ  ← δbg

        # ----- Process noise input Jacobian G (15×12) -----
        # Noise vector n = [n_gyro(3), n_accel(3), n_bg(3), n_ba(3)]
        G = np.zeros((15, 12))
        G[3:6, 3:6]   = self.R      # accel noise  → δv
        G[6:9, 0:3]   = np.eye(3)   # gyro  noise  → δθ
        G[9:12, 6:9]  = np.eye(3)   # gyro  bias RW
        G[12:15, 9:12] = np.eye(3)  # accel bias RW

        # ----- Discrete process noise Q (12×12) -----
        # Noise spectral density → per-step covariance
        Q = np.zeros((12, 12))
        ...
        # Covariance propagation: P = F P Fᵀ + G Q Gᵀ
        self.P = F @ self.P @ F.T + G @ Q @ G.T

        # Update nominal state; re-orthogonalise R
        self.p = p_new
        self.v = v_new
...
class LioNode(Node):
    ...
    def cb_imu(self, msg: Imu):
        ...
        if not self.ieskf.gravity_initialized:
            ...
            # Only propagate after the first scan so that the initial state
            # is anchored at origin before any IMU drift accumulates.
    ...
    def cb_cloud(self, msg: PointCloud2):
        ...
        if (self.use_imu
                and self.ieskf.gravity_initialized
