...
def pointcloud2_to_xyz_i(msg: PointCloud2) -> Tuple[np.ndarray, np.ndarray]:
    """Fast numpy-based PointCloud2 extraction — no Python point-iteration."""
    ...
    if 'intensity' in field_offsets:
        intensity = _col(field_offsets['intensity'])
    else:
...
class NonRepetitiveLiDARRos2Node(Node):
    def __init__(self):
        ...
        self.R_world: np.ndarray = np.eye(3, dtype=float)  # full SO(3) rotation for odometry
    ...
    def _publish_odom_and_tf(self, stamp, final_pose: np.ndarray, st: Optional[ScanState]):
        ...
        qx, qy, qz, qw = rot_to_quat(self.R_world)
    ...
    def cb_cloud(self, msg: PointCloud2):
        ...
        try:
            ...
            if self.prev_cloud is not None and len(self.prev_cloud.points) > 0:
                ...
                # Re-orthogonalize and store full 3-D rotation for odometry quaternion
                U, _, Vt = np.linalg.svd(self.cumulative_transform[:3, :3])
                self.R_world = U @ Vt
            ...
            # Accumulate intensity-coloured map.
            # In 2-D mode keep yaw-only rotation so z-drift from GICP can't
            # contaminate the flat map.  In 3-D mode use the full SO(3) from
            # the cumulative transform so pitch/roll are correctly applied.
            if self.force_z_zero:
                ...
                Tmap = self.cumulative_transform.copy()
